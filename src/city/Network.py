...
class Streets:
	...
	def buffer(self, width_column='width'):
		...
		for w in gdf[width_column].unique():
			gdf.loc[gdf[width_column] == w, 'geometry'] = gdf.loc[gdf[width_column] == w, 'geometry'].buffer(w / 2,
			                                                                                                 cap_style=2)
		gdf['Area'] = gdf.area
		return gdf
		...
		if self.widths is not None:
			...
			for i in widths.index:
				if '(m)' in str(widths.at[i, 'width']):
					...
					try:
						widths.at[i, 'width'] = float(widths.loc[i, 'width'].split('R')[0])
					except:
						widths = widths.drop(i)
				elif 'M' in str(widths.at[i, 'width']):
					widths.at[i, 'width'] = float(widths.loc[i, 'width'].split('M')[0])
			...
			for use in joined['streetuse'].unique():
				joined.loc[(joined['streetuse'] == use) & (joined['width'].isna()), 'width'] = \
	...
	def visibility(self):
		if self.barriers is not None:
			...
			gdf.drop(['geometry'], axis=1).set_geometry('isovists').to_file(f'{directory}/isovists.geojson',
			                                                                driver='GeoJSON')
	...
	def greenery(self):
		...
		if self.trees is not None:
			...
			if 'width' not in gdf.columns:
				...
				buffer = gpd.GeoDataFrame([g.buffer(w / 2) for g, w in zip(gdf['geometry'], gdf['width'])],
				                          columns=['geometry'], crs=26910)
...
class Network:
	...
	def build(self):
		...
		edges = edges[edges['to'].isin(nodes['osmid']) & edges['from'].isin(nodes['osmid'])]

		# Assign a number for every unique osmid string
		replacements = {un: i for i, un in enumerate(set(sorted(
			list(nodes['osmid'].unique()) + list(edges['to'].unique()) + list(edges['from'].unique()))))}
		...
		assert nodes.crs == edges.crs, AttributeError('Coordinates systems of edges and nodes are different')
		...
		net = pdna.Network(nodes['x'], nodes['y'], edges["from"], edges["to"], edges[["length"]], twoway=True)
