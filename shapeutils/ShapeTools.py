...
def divide_line_by_count(line, n_segments):
	...
	splitters = [MultiPoint([snap(pt, ln, tolerance=0) for pt in points]).buffer(0.001) for points, ln in
				 zip(splitters, line)]
	return [MultiLineString([segment for segment in split(ln, splitter) if segment.length > 0.1]) for ln, splitter in
			zip(line, splitters)]
...
class Shape:
	...
	def is_convex(self):
		...
		self.gdf['convex'] = [False if geom.minimum_rotated_rectangle.area > 1.02 * (geom.area) else True for geom in
							  self.gdf.geometry]
		return self.gdf
	...
	def reduce(self, buffer=5):
		...
		if 'Multi' in uu.geom_type:
			...
			return gpd.GeoDataFrame({'geometry': [self.gdf.copy().buffer(1).unary_union.buffer(-buffer)]},
									crs=self.gdf.crs)
	...
	def dissolve(self, tolerance=0.1, join_operations=None, min_area=1):
		...
		uu = self.gdf.copy().buffer(tolerance, join_style=2, cap_style=2).unary_union.buffer(-tolerance, join_style=2,
																							 cap_style=2)
		...
		if join_operations is not None:
			gdf = SpatialAnalyst(gdf, self.gdf.copy()).spatial_join(operations=join_operations)
		...
		gdf['parent_id'] = [int(i) for i in
		                    SpatialAnalyst(gdf, self.gdf.copy().loc[:, ['id', 'geometry']]).spatial_join(operations=['max'])[
								'id_max']]
		gdf = gdf.set_geometry('geometry')
	...
	def move(self, reference_geom):
		...
		gdf['geometry'] = [translate(geom, xoff=ref_ctr.x - gdf_ctr.x, yoff=ref_ctr.y - gdf_ctr.y) for geom in
						   gdf['geometry']]
		return gdf
	...
	def flip(self, x, y, reference_geom=None):
		...
		gdf = self.gdf.copy()
		gdf['geometry'] = [scale(geom, xfact=x, yfact=y, origin=reference_geom.unary_union.centroid) for geom in
						   gdf['geometry']]
	...
	def min_rot_rec(self, quick_run=False):
		...
		gdf['largest_parallel'] = [geom[i[1]] if geom is not None else None for i, geom in
								   zip(largest, gdf['segments'])]
		gdf['smallest_segment'] = [geom[i[0]] if geom is not None else None for i, geom in
								   zip(smallest, gdf['segments'])]
		gdf['smallest_parallel'] = [geom[i[1]] if geom is not None else None for i, geom in
									zip(smallest, gdf['segments'])]
		...
		if not quick_run:
			...
			# Get diagonal line
			gdf['diagonal1'] = [LineString([Point(ln.coords[0]), Point(ln.coords[2])]) if ln is not None else None for
								ln in gdf['min_rot_rec']]
			gdf['diagonal2'] = [LineString([Point(ln.coords[1]), Point(ln.coords[3])]) if ln is not None else None for
								ln in gdf['min_rot_rec']]
		...
		for i in gdf.index:
			...
			while max_geom_area > max_area:
				...
				if geometry.area > max_area:
					...
					min_rot_rec = Shape(gpd.GeoDataFrame({'geometry': [geometry]}, geometry='geometry')).min_rot_rec(
						quick_run=True)  # !!!
	...
	def subdivide_offset(self, width=30, depth=12, outwards=False):
		...
		if len(inner) > 0:
			if not outwards:
				...
				outer['divider'] = [
					MultiPoint([nearest_points(pt, outer_ln)[1] for pt in mpt]) if outer_ln.length > 0 else None
				...
				inner['divider'] = [
					MultiPoint([nearest_points(pt, inner_ln)[1] for pt in mpt]) if inner_ln.length > 0 else None
			...
			gdf['splitters'] = [
				MultiLineString([LineString([pt0, pt1]) for pt0, pt1 in zip(mpt0, mpt1)]) if mpt1 is not None else None
				for
...
class SpatialAnalyst:
	...
	def __check_crs(self):
		if not self.left_gdf.crs == self.right_gdf.crs:
			raise AttributeError(f'Left ({self.left_gdf.crs}) and right ({self.right_gdf.crs}) coordinate systems are different')
		return

	def __clean_column_names(self):
		self.left_gdf.columns = [col if '/' in col else col.replace('/', '_') for col in self.left_gdf.columns]
		self.right_gdf.columns = [col if '/' in col else col.replace('/', '_') for col in self.right_gdf.columns]
		return
...
if __name__ == '__main__':
	bi_variate_map(
		gpd.read_file('/Volumes/ELabs/50_projects/19_SSHRC_Insight/02_shading_model/Results_shp/roofs/roofs.shp'),
